...
test('returns expected query result object (array bytea)', async (t) => {
    const pool = src_1.createPool(t.context.dsn);
    const result = await pool.query(src_1.sql `
    SELECT ${src_1.sql.array([Buffer.from('foo')], 'bytea')} "names"
  `);
    t.deepEqual(result, {
        command: 'SELECT',
        fields: [
...
test('Tuple moved to another partition due to concurrent update error handled', async (t) => {
    ...
    await pool.connect(async (connection1) => {
        await pool.connect(async (connection2) => {
            await connection1.query(src_1.sql `BEGIN`);
            await connection2.query(src_1.sql `BEGIN`);
            await connection1.query(src_1.sql `UPDATE foo SET a = 2 WHERE a = 1`);
            connection2.query(src_1.sql `UPDATE foo SET b = 'XYZ'`).catch((error) => {
                var _a;
            ...
            await delay_1.default(1000);
            await connection1.query(src_1.sql `COMMIT`);
            await connection2.query(src_1.sql `COMMIT`);
        });
    });
    await pool.end();
});

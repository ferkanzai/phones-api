...
ava_1.default('rollsback unsuccessful transaction', async (t) => {
    const pool = createPool_1.createPool();
...
ava_1.default('uses savepoints to nest transactions', async (t) => {
    const pool = createPool_1.createPool();
    await pool.connect((c1) => {
...
ava_1.default('rollsback to the last savepoint', async (t) => {
    const pool = createPool_1.createPool();
...
ava_1.default('rollsback the entire transaction with multiple savepoints', async (t) => {
    const pool = createPool_1.createPool();
    await pool.connect((c1) => {
...
ava_1.default('rollsback the entire transaction with multiple savepoints (multiple depth layers)', async (t) => {
    const pool = createPool_1.createPool();
    await pool.connect((c1) => {
        return t.throwsAsync(c1.transaction(async (t1) => {
            await t1.query(sql `SELECT 1`);
...
ava_1.default('throws an error if an attempt is made to create a new transaction before the last transaction is completed', async (t) => {
    const pool = createPool_1.createPool();
...
ava_1.default('throws an error if an attempt is made to execute a query using the parent transaction before the current transaction is completed', async (t) => {
    const pool = createPool_1.createPool();
    const connection = pool.connect((c1) => {
        return c1.transaction((t1) => {
